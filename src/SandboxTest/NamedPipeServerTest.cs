...
namespace SandboxTest
{
    public class TestNamedPipeServer
    {
        ...
        {
            Assert.Throws< ArgumentException >( () => new NamedPipeServer( Mock.Of< INamedPipeStreamFactory >(), string.Empty ) );
        }

        [Fact]
        public void TestNullAddressMustThrowArgNullEx()
        {
            Assert.Throws< ArgumentNullException >( () => new NamedPipeServer( Mock.Of< INamedPipeStreamFactory >(), null ) );
        }

        [Fact]
        public void TestNullFactoryMustThrowArgNullEx()
        {
            Assert.Throws< ArgumentNullException >( () => new NamedPipeServer( null, NotEmptyAddress ) );
        }
        ...
        public void TestCanSubscribeOnMessagesMustCallConnectWithCorrectAddress()
        {
            ...
            stream.Setup( it => it.ConnectionAsync( It.Is< CancellationToken >( ct => ct != null && ct != CancellationToken.None ) ) ).Returns( Extentions.CompletedTask );
        ...
        public void TestProtocolInFirstReadLengthThenMessage( string messageString )
        {
            ...
            {
                var stream = new Mock< INamedPipeStream >();
                stream.InSequence( seq ).Setup( it => it.ConnectionAsync( It.IsAny< CancellationToken >() ) ).Returns( Extentions.CompletedTask );
                stream.InSequence( seq ).
                       Setup( it => it.ReadAsync( It.Is< byte[] >( ar => ar.Length == sizeof( int ) ), It.Is< int >( o => o == 0 ), It.Is< int >( c => c == sizeof( int ) ),
                ...
                stream.InSequence( seq ).
                       Setup( it => it.ReadAsync( It.Is< byte[] >( ar => ar.Length == message.Length ), It.Is< int >( o => o == 0 ), It.Is< int >( c => c == message.Length ),
                                  It.Is< CancellationToken >( ct => ct != null && ct != CancellationToken.None ) ) ).
                       Returns< byte[], int, int, CancellationToken >( ( bytes, offset, count, ct ) =>
                                                                       {
        ...
        {
            var stream = new Mock< INamedPipeStream >();
            stream.Setup( it => it.ConnectionAsync( It.Is< CancellationToken >( ct => ct != null && ct != CancellationToken.None ) ) ).Throws< TestException >();
            var streamFactory = new Mock< INamedPipeStreamFactory >();
            streamFactory.Setup( it => it.CreateStream( It.IsAny< string >() ) ).Returns( stream.Object );
            var observer = new Mock< IObserver< byte[] > >();
            using ( new NamedPipeServer( streamFactory.Object, NotEmptyAddress ).Subscribe( observer.Object ) )
        ...
        [Fact]
        public void TestReadLengthWithZeroByteCountMustThrowProcessTerminatedException()
        {
            var stream = new Mock< INamedPipeStream >();
            stream.Setup( it => it.ConnectionAsync( It.IsAny< CancellationToken >() ) ).Returns( Extentions.CompletedTask );
            stream.Setup( it => it.ReadAsync( It.IsAny< byte[] >(), 0, It.IsAny< int >(), It.IsAny< CancellationToken >() ) ).Returns( Task.FromResult( 0 ) );
            var streamFactory = new Mock< INamedPipeStreamFactory >();
            streamFactory.Setup( it => it.CreateStream( It.IsAny< string >() ) ).Returns( stream.Object );
            var observer = new Mock< IObserver< byte[] > >();
            using ( new NamedPipeServer( streamFactory.Object, NotEmptyAddress ).Subscribe( observer.Object ) )
        ...
        [Fact]
        public void TestReadMessageWithZeroByteCountMustThrowProcessTerminatedException()
        {
            var seq = new MockSequence();
            var stream = new Mock< INamedPipeStream >();
            stream.InSequence( seq ).Setup( it => it.ConnectionAsync( It.IsAny< CancellationToken >() ) ).Returns( Extentions.CompletedTask );
            ...
            stream.InSequence( seq ).Setup( it => it.ReadAsync( It.IsAny< byte[] >(), 0, It.IsAny< int >(), It.IsAny< CancellationToken >() ) ).Returns( Task.FromResult( 0 ) );
            var streamFactory = new Mock< INamedPipeStreamFactory >();
            streamFactory.Setup( it => it.CreateStream( It.IsAny< string >() ) ).Returns( stream.Object );
            var observer = new Mock< IObserver< byte[] > >();
            using ( new NamedPipeServer( streamFactory.Object, NotEmptyAddress ).Subscribe( observer.Object ) )
        ...
        [Fact]
        public void TestStreamMustBeDisposedAfterUnsubscribeAllClients()
        {
            var (stream, namedPipeServer) = CreateStreamAndServerForPublishTest();
            namedPipeServer.Subscribe( it => { } ).Dispose();
            stream.Verify( it => it.Dispose(), Times.Once );
        }

        private static Func< byte[], int, int, CancellationToken, Task< int > > CompleteOnCancel()
        {
            return ( bytes, offset, count, ct ) =>
                   {
                       var cts = new TaskCompletionSource< int >();
                       ct.Register( () => cts.SetResult( 10 ) );
                       return cts.Task;
                   };
        }
        ...
        private static (Mock< INamedPipeStream > streamMock, NamedPipeServer server) CreateStreamAndServerForPublishTest()
        {
            var stream = new Mock< INamedPipeStream >();
            stream.Setup( it => it.ConnectionAsync( It.Is< CancellationToken >( ct => ct != null && ct != CancellationToken.None ) ) ).Returns( Extentions.CompletedTask );
            stream.Setup( it => it.WriteAsync( It.IsAny< byte[] >(), 0, It.IsAny< int >(), It.IsAny< CancellationToken >() ) ).Returns( Extentions.CompletedTask );

            stream.Setup( it => it.ReadAsync( It.IsAny< byte[] >(), 0, It.IsAny< int >(), It.IsAny< CancellationToken >() ) ).Returns( CompleteOnCancel() );
